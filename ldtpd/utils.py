...
class ProcessStats(threading.Thread):
    ...
    def get_cpu_memory_stat(self):
        ...
        for i in statgrab.sg_get_process_stats():
            ...
            if not re.search(re.escape(self._appname), proctitle,
                             re.U | re.L):
            ...
            if event and (event.type == "window:destroy" or \
                ...
                if abbrev_name == '':
                    for win_name in self._appmap.keys():
                        ...
                        if re.search('%s\d*$' % re.escape(abbrev_role), win_name, re.M | re.U):
                    ...
                    for name in self._appmap.keys():
                        ...
                        if re.search('%s%s\d*$' % (re.escape(abbrev_role), re.escape(abbrev_name)),
                                     win_name, re.M | re.U) or \
                                     re.search('%s%s*$' % (re.escape(abbrev_role), re.escape(abbrev_name)),
                                               win_name, re.M | re.U):
