...
if 'ONLY_LDTP_DEBUG' in os.environ:
    _ldtp_debug = os.environ['ONLY_LDTP_DEBUG']
...
# def debugOn():
#     _ldtp_debug = 1
#     client._client.setDebug(1)
#     _restartPopulateNamespace(globals())
#
# def debugOff():
#     _ldtp_debug = 0
#     client._client.setDebug(0)
#     _restartPopulateNamespace(globals())
...
_logger_default_level = logger.level
...
def startlog(filename, overwrite = True):
    ...
    if overwrite:
        ...
        _mode = 'a'
    global _file_logger, _ldtp_debug
    ...
    if _ldtp_debug:
        ...
        logger.setLevel(_logger_default_level)
...
# set_methods = set()
#
# def _restartPopulateNamespace(d):
#     for method, local_name in set_methods:
...
def _populateNamespace(d):
    for method in client._client.system.listMethods():
        ...
        if method in d:
            ...
            local_name = method
        # set_methods.add((method, local_name))
        d[local_name] = getattr(client._client, method)
        d[local_name].__doc__ = client._client.system.methodHelp(method)
