...
elif 'LDTP_ONLY_DEBUG' in os.environ:
    _ldtp_debug = os.environ['LDTP_ONLY_DEBUG']
else:
    _ldtp_debug = None
...
class _Method(xmlrpclib._Method):
    # _debug = _ldtp_debug
    #
    # def __init__(self, send, name, debug):
    #     self._debug = debug
    #     super(_Method, self).__init__(send, name)
    #
...
class LdtpClient(xmlrpclib.ServerProxy):
    ...
    def __init__(self, uri, encoding=None, verbose=0, use_datetime=0):
        ...
        xmlrpclib.ServerProxy.__init__(
            # self, uri, self.transport, encoding, verbose, 1, use_datetime)
            self, uri, Transport(), encoding, verbose, 1, use_datetime)

    def __getattr__(self, name):
        # magic method dispatcher
        # return _Method(self._ServerProxy__request, name, self._debug)
